...
--*/
{
    ...
    case IOCTL_DISK_GET_PARTITION_INFO: {
        ...
        break;
	case IOCTL_WRITE_DISKIMAGE_FILE: {
		HANDLE file = NULL;
		OBJECT_ATTRIBUTES object;
		UNICODE_STRING fileName;
		IO_STATUS_BLOCK ioStatBlock;
		LARGE_INTEGER ByteOffset = {0};
		PUCHAR buffer = ExAllocatePoolWithTag(NonPagedPool, (1024*1024), 'RMDK');  
		if (buffer == NULL) {
		...
		KdPrint(("Start writing!\n"));

		RtlCopyMemory(buffer, devExt->DiskImage, (1024*1024));
		RtlInitUnicodeString(&fileName, L"\\DosDevices\\C:\\file_w.txt");
		InitializeObjectAttributes(&object, &fileName, OBJ_CASE_INSENSITIVE | OBJ_KERNEL_HANDLE, NULL, NULL);
		Status = ZwCreateFile(&file, FILE_APPEND_DATA, &object, &ioStatBlock, NULL, FILE_ATTRIBUTE_NORMAL,
							  FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE, FILE_OVERWRITE_IF,
							  FILE_SYNCHRONOUS_IO_NONALERT, NULL, 0);

		if (!NT_SUCCESS(Status)) {
			KdPrint(("Error while creating file! Status: 0x%0.8x, IoStatusBlock: 0x%0.8x\n", Status, ioStatBlock.Status));
			ExFreePoolWithTag(buffer, 'RMDK');
			break;
		}

		Status = ZwWriteFile(file, NULL, NULL, NULL, &ioStatBlock, buffer, (1024*1024), &ByteOffset, NULL);

		if (!NT_SUCCESS(Status)) {
			KdPrint(("Error while writing! Status: 0x%0.8x\n", Status));
		}

		ZwClose(file);
		ExFreePoolWithTag(buffer, 'RMDK');

		KdPrint(("Writing was complete!\n"));

		information = (1024*1024);
		Status = STATUS_SUCCESS;
	}
	break;

	case IOCTL_READ_DISKIMAGE_FILE: { 
		HANDLE file; 
		OBJECT_ATTRIBUTES object; 
		UNICODE_STRING fileName; 
		IO_STATUS_BLOCK ioStatBlock; 
		...
		PUCHAR buffer = ExAllocatePoolWithTag(NonPagedPool, (1024*1024), RAMDISK_TAG); 	 	 	 	 	   
		KdPrint(("Start reading!\n")); 

		RtlInitUnicodeString(&fileName, L"\\??\\C:\\file.txt"); 
		InitializeObjectAttributes(&object, 	&fileName, 	OBJ_KERNEL_HANDLE, NULL, NULL); 
		Status = ZwOpenFile(&file, FILE_READ_DATA, &object, &ioStatBlock, FILE_SHARE_READ, FILE_NON_DIRECTORY_FILE); 

		if (!NT_SUCCESS(Status)) {
		    KdPrint(("Error while open file! 0x%0.8x, 0x%0.8x\r\n", Status, ioStatBlock.Status));
		}
		
		Status = ZwReadFile(file, NULL, NULL, NULL, &ioStatBlock, buffer, (1024*1024), &ByteOffset, NULL);
							
		if (!NT_SUCCESS(Status)) {
		    KdPrint(("Error while reading! 0x%0.8x, 0x%0.8x\r\n", Status, ioStatBlock.Status)); 
		}
		
		ZwClose(file); 	 
		RtlCopyMemory(devExt->DiskImage, buffer, (1024*1024)); 

		KdPrint(("Reading was complete!\n")); 
...
--*/
{
	...
	KdPrint(("RamDiskEvtDeviceContextCleanup begins\n"));	
